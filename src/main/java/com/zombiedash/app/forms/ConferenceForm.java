...
public class ConferenceForm {
  ...
  private static final int MAX_ATTENDEE_CHARACTER_LENGTH_LIMIT = 6;
  private static final int VENUE_LENGTH_LIMIT = 200;
  private static final int DESCRIPTION_LENGTH_LIMIT = 500;
  private static final int TOPIC_LENGTH_LIMIT = 100;
  ...
  public boolean isValidData() {
    ...
    validDataFlag &= (conf_name.length() <= NAME_LENGTH_LIMIT);
    validDataFlag &= (conf_topic.length() <= TOPIC_LENGTH_LIMIT);
    validDataFlag &= (conf_description.length() <= DESCRIPTION_LENGTH_LIMIT);
    validDataFlag &= (conf_venue.length() <= VENUE_LENGTH_LIMIT);
    validDataFlag &= (conf_max_attendees.length() <= MAX_ATTENDEE_CHARACTER_LENGTH_LIMIT);
